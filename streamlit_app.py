...
def render_step_images(step: dict) -> None:
    image_paths = []

    if step.get("image"):
        image_paths.append(step["image"])

    if step.get("images"):
        image_paths.extend(step["images"])

    image_width = int(step.get("image_width", DEFAULT_IMAGE_WIDTH))
